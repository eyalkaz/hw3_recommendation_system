...
class collaborative_filtering:
    ...
    def create_user_based_matrix(self, data):
        ...
        self.user_based_matrix = 1 - pairwise_distances(self.user_item_matrix, metric='cosine')  # cosine similarity = 1 - cosiine distance

        # for predilection
        if not self.movies_rating_matrix:  # movies rating matrix not created yet
    ...
    def create_item_based_matrix(self, data):
        ...
        if not self.mean_user_rating:  # we didnt call user item matrix
        ...
        self.movies_rating_matrix = self.movies_rating_matrix.sub(self.movies_rating_matrix.mean(axis=0), axis=1).fillna(0)
        self.item_based_metrix = 1 - pairwise_distances(self.movies_rating_matrix, metric='cosine')  # cosine similarity = 1 - cosiine distance

        # for predilection
        if not self.user_item_matrix:  # user rating matrix not created yet
            self.user_item_matrix = self.movies_rating_matrix.T

    def predict_movies(self, user_id, k, is_user_based=True):
        "*** YOUR CODE HERE ***"
        user_id = int(user_id)
        self.dict_users = {k: v for v, k in enumerate(self.user_item_matrix.index)}  # flipped dictionary with user : index
        self.dict_items = dict(enumerate(self.movies_rating_matrix.index))  # index : movie
        if is_user_based:
        ...
        candidates = prediction_matrix[self.dict_users[user_id]]
        ...
        top_k_movies_id = [self.dict_items[i] for i in top_k]
